...
def gen_prompt_balanced(
    N, names, nouns_dict
):
    ioi_prompts = []
    for t in range(len(ABBA_TEMPLATES)):
        # generate ABBA templates
        for n in range(N):
            temp = ABBA_TEMPLATES[t]
            temp_id = ABBA_TEMPLATES.index(temp)
            name_1 = ""
            name_2 = ""
            # avoid duplicate names
            while len(set([name_1, name_2])) < 2:
                name_1 = random.choice(names)
                name_2 = random.choice(names)
            nouns = {}
            ioi_prompt = {}
        ...
        # generate BABA templates
        for n in range(N):
            temp = BABA_TEMPLATES[t]
            temp_id = BABA_TEMPLATES.index(temp)
            name_1 = ""
            name_2 = ""
            # avoid duplicate names
            while len(set([name_1, name_2])) < 2:
...
class IOIDataset:
    def __init__(
        ...
        if prompt_type == "ABBA":
            ...
            random.shuffle(self.templates)
        elif prompt_type == "balanced":
            self.templates = "balanced"
        ...
        if prompts is None and prompt_type == "balanced":
            self.ioi_prompts = gen_prompt_balanced(
                N=100,
                names=NAMES,
